...
def handle_dialog(req, res):
    ...
    if req['request']['original_utterance'].lower() in [
        ...
        res['response']['text'] = 'Рядом с вами есть две пицерии, какую вы выбирете?'
        
        if req['request']['original_utterance'].lower() in [
        'Яндекс пицца',
        'Гугл пицца'
        ]:
            # Пользователь выбрал пицерию, предоставляем ингридиенты.
            res['response']['text'] = 'На яндекс Пицце можно заказать: Четыре сыра и Гавайскую пиццу. На Гугл Пицце можно заказать: Гавайскую пиццу и Маргариту'
        
            if req['request']['original_utterance'].lower() in [
            'Я хочу заказать Гавайскую пиццу с Яндекс пиццы'
            ]:                
                # Пользователь согласился, прощаемся.
                res['response']['text'] = 'Ваш заказ собран, оформить его можно на Яндекс пицце'
                res['response']['end_session'] = True
                return
            
            if req['request']['original_utterance'].lower() in [
            'Я хочу заказать Маргариту с Гугл пиццы'
            ]:
                # Пользователь согласился, прощаемся.
                res['response']['text'] = 'Ваш заказ собран, оформить его можно на Гугл пицце'
                res['response']['end_session'] = True
                return
